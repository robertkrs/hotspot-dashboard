<?php
  ...
  /**verifica o numero de pessoas online - CARD 1*/
    ...
    if(!empty($_GET['search'])){
      $data = $_GET['search'];
      $sql5 = "SELECT * FROM administracao WHERE id_adm LIKE '%$data%' OR login_adm  LIKE '%$data%'";
    }
    else{
...
<head>
  ...
  <title>Administradores | Signet</title>
      ...
      <div class="details-minha-conta">
          ...
          <table>
            ...
            <?php 
              ...
              while($user_data = mysqli_fetch_assoc($result5)){
                ...
                echo '<tr>';
                echo '<td>'.$user_data['id_adm'];'</td>';
                echo '<td>'.$user_data['login_adm'];'</td>';
                $nivel = 2;
                $login = $_SESSION['login'];
                $sql7 = "SELECT * FROM administracao INNER JOIN role_permissions ON administracao.role_id = role_permissions.role_id where perm_id = '$nivel' and login_adm = '$login'";
                $result9 = $conexao->query($sql7);
                if(mysqli_num_rows($result9)){
                  echo "<td>
                  <a href='./../cadastros-sub/editar_adm.php?id=$user_data[id_adm]'><ion-icon src='/../../../img/geral/editar.svg'></ion-icon></a></td>";
                }
                $nivel = 4;
                $sql7 = "SELECT * FROM administracao INNER JOIN role_permissions ON administracao.role_id = role_permissions.role_id where perm_id = '$nivel' and login_adm = '$login'";
                $result9 = $conexao->query($sql7);
                if(mysqli_num_rows($result9)){
                  echo "<td>
                  <a href='./../cadastros-sub/remover_adm.php?id=$user_data[id_adm]' onclick='return (confirm(\"Quer mesmo excluir esse registro?\"))'><ion-icon src='/../../../img/geral/remover.svg'></ion-icon</a></td>";
                }
                echo '</tr>';
        ...
        if(mysqli_num_rows($result9)){
          ...
          <div class="cadastrar-plus">
            <a href="../cadastros-sub/cadastro_adm.php"><ion-icon src="/../../../img/geral/plus.svg"></a>
          </div>
          ';
        }
        ?>
