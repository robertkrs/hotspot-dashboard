<?php
  ...
  if(!empty($_GET['id'])){
    ...
    $id = $_GET['id'];
    $sqlSelect = "SELECT * FROM administracao where id_adm = $id";
    $result = $conexao->query($sqlSelect);

    if($result->num_rows > 0){
      while($user_data = mysqli_fetch_assoc($result)){
       
        $email = $user_data['login_adm'];
        $senha = $user_data['senha_adm'];
    ...
    <main class="main">
      ...
      <div class="adm-registro">
        <div class="recentOrders">
          ...
          <form action="alter_adm.php" method="POST" class="adm-formulario ad-formulario">
          
          <div>
            <input type="email" name="email" id="email" placeholder="email" value="<?php echo $email ?>">
          </div>
          <div>
            <input type="password" name="senha" id="senha" placeholder="nova senha" value="<?php echo $senha ?>">
          </div>
          <div>
            <select name="role" id="role">
              ...
              <?php
                  ...
                  $resultR = $conexao->query($sqlR);
                  while($user_data2 = mysqli_fetch_assoc($resultR)){
                    echo "<option value='$user_data2[role_id]'>$user_data2[role_name]</option>";
                  }
                ?>
              </optgroup>
            </select>
