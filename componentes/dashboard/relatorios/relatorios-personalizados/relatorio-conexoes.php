...
<?php	
  ...
  $conexoes_usuarios = " SELECT * FROM radacct inner join cadastro_clientes on radacct.username = cadastro_clientes.email_cliente WHERE datediff(cast(date(now())as date),cast(acctupdatetime as date)) < '$dias' and email_cliente like '%$cadastro%' and framedipaddress like '%$ip%'  group by acctupdatetime";
	...
	$dompdf->load_html('
      ...
      .relatorio tr td{
        ...
        font-size:0.75rem;
			...
			<h1 style="text-align: center;">Relatório de Conexões - Personalizado</h1>
			'. $html .'
		');
